...
static inline void set_linear_poly_21(nvars_t nlins, uint32_t *lineqs,
                                      nvars_t *linvars, ht_t *bht,
                                      int32_t *bexp_lm, bs_t *bs) {
  ...
  for (int i = 0; i < nv; i++) {
    if (linvars[i] != 0) {
      ...
      if (len == bht->nv + 1) {
        for (long j = 0; j < len; j++) {
          uint32_t coef = bs->cf_21[bs->hm[bi][COEFFS]][j];
          lineqs[cnt * (nv + 1) + j] = coef;
        }
      } else {
        hm_t *dt = bs->hm[bi] + OFFSET;
...
static inline void check_and_set_linear_poly_21(nvars_t *nlins_ptr,
                                                nvars_t *linvars,
                                                uint32_t **lineqs_ptr,
                                                ht_t *bht, int32_t *bexp_lm,
                                                bs_t *bs) {
